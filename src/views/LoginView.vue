    ...
    <div class="flex flex-col items-center justify-center gap-5 text-xl">
      ...
      >
        ...
        <RouterLink
          ...
          to="/registration"
          >Not yet have an account</RouterLink
        >
      </form>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  const userData = ref<{ password: string; username: string }>({
    password: '',
    username: ''
  })
  const handleLogin = () => {
    login(userData.value.username, userData.value.password).then(() => router.push('/'))
  }
</script>
